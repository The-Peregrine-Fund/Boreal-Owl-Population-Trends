...
ps1 <- MCMCpstr(m.site1[[7]]$samples, pars, type="chains")
...
coeftab.s1.r <- MCMCsummary(m.site1[[7]]$samples, pars, 
                            HPD=TRUE,  hpd_prob = 0.95,
                            round=2, pg0 = TRUE, 
                            func = median, func_name = "median")
...
for (t in 1:length(yr)){
  l.ab2[t, ] <- ps1$mu + 
                ...
                ps1$delta[2,]*cos(2*pi*cycle.sc2[t]) + 
                ps1$delta[3,]*sin(2*pi*cycle.sc2[t]) +
                ps1$delta[4,]*cos(2*pi*cycle.sc2[t])*yr.sc[t] + 
                ps1$delta[5,]*sin(2*pi*cycle.sc2[t])*yr.sc[t] 
