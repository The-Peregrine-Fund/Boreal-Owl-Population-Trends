...
constl <- list(time = (dat2$year2 - median(1994:2023))/ max(dat2$year2 - median(1994:2023)),
...
constl$time2 <- 1:length(1994:2023)/4
  ...
  code <- nimbleCode(
    {
      ...
      for (t in 1:ntime){
        ...
        log(lambda[t]) <- mu[season[t]] + log(effort[t]) + 
          delta[1]*time[t] + 
          delta[2]*time[t]^2 +
...
save(out=zip, post=post,
     file="C:\\Users\\rolek.brian\\OneDrive - The Peregrine Fund\\Documents\\Projects\\Boreal Owl pop trends\\outputs\\zip-site2.Rdata")
