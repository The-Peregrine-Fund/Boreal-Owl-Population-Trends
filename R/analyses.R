...
##################
  ...
  params <- c( "mu", "delta", "r", "p", "lambda", "logProb_y")
  ...
  n.chains=4; n.thin=10; n.iter=20000; n.burnin=10000
  mod <- nimbleModel(code, calculate=T, constants = constl, 
                     data = datl, inits = inits(), 
                     buildDerivs = TRUE)
  mod$calculate()
...
##################  
  ...
  params <- c( "mu", "r", "p", "lambda", "logProb_y")
  # create initial values for missing y data
  inits <- function(){ list (mu = runif(1, -2, 2),
                             r = runif(1, 0, 1),
                             lambda = rpois(constl$ntime, mean(datl$y)) 
...
df.waic <- data.frame( 
            model = 1:6,
            name = c('seas+year+year2+seas:year+seas:year2',
                     'seas+year+year2+seas:year', 
                     'seas+year+year2', 
                     'seas+year', 
                     'seas', 
                     'null'),
...
save(m=m, looc=looc, df.waic=df.waic,
     file="C:\\Users\\rolek.brian\\OneDrive - The Peregrine Fund\\Documents\\Projects\\Boreal Owl pop trends\\outputs\\full_model_set.Rdata")
